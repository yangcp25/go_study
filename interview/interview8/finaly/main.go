...
type Conn struct {
	conn       net.Conn
	pipe       map[string]*io.PipeWriter
	writeLock  sync.Mutex
	readerLock sync.Mutex
	coming     chan DataComing
}
type DataComing struct {
	key    string
	reader io.Reader
}
...
func (w Writer) Write(data []byte) (n int, err error) {
	...
	// 写入数据类型
	err = binary.Write(w.conn.conn, binary.BigEndian, FrameData)
	if err != nil {
		return
	}
	// 写入keyLen key
	err = binary.Write(w.conn.conn, binary.BigEndian, uint16(len(w.key)))
	if err != nil {
		return
	}
	_, err = w.conn.conn.Write([]byte(w.key))
	if err != nil {
		return
	}
	// 写入数据长度 数据
	err = binary.Write(w.conn.conn, binary.BigEndian, int64(len(data)))
	if err != nil {
		return
	}
	n, err = w.conn.conn.Write(data)
	if err != nil {
		return
	}
	return
...
func (w Writer) Close() (err error) {
	...
	defer w.conn.writeLock.Unlock()
	// 写入数据类型
	err = binary.Write(w.conn.conn, binary.BigEndian, FrameEnd)
	if err != nil {
		return
	}
	// 写入keyLen key
...
func (conn *Conn) Send(key string) (writer io.WriteCloser, err error) {
	return &Writer{conn: conn, key: key}, nil
}
...
func NewConn(conn net.Conn) *Conn {
	dataConn := &Conn{
		conn:   conn,
		pipe:   make(map[string]*io.PipeWriter),
		coming: make(chan DataComing),
	}
	go DataLoop(dataConn)
	return dataConn
}

func DataLoop(conn *Conn) {
	defer func() {
		conn.readerLock.Lock()
		for _, reader := range conn.pipe {
			reader.Close()
		}
		conn.readerLock.Unlock()
		close(conn.coming)
	}()
	for {
		// 读取类型
		...
		err = binary.Read(conn.conn, binary.BigEndian, &keyLength)
		if err != nil {
			return
		}
		var key = make([]byte, keyLength)
		...
		case FrameData:
			...
			err = binary.Read(conn.conn, binary.BigEndian, &dataLength)
			if err != nil {
				return
			}
			conn.readerLock.Lock()
			...
			if v, ok := conn.pipe[keyStr]; ok {
				pw = v
			} else {
				pr, pw = io.Pipe()
				conn.pipe[keyStr] = pw
				conn.coming <- DataComing{key: keyStr, reader: pr}
			}
			conn.readerLock.Unlock()
			_, err := io.CopyN(pw, conn.conn, dataLength)
			if err != nil {
				fmt.Println(err)
			}
		case FrameEnd:
