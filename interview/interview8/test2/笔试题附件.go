...
type Conn struct {
	...
	Reader     map[string]io.PipeReader
	DataComing chan incoming
}
...
// 自定义数据帧类型
const (
	FrameData uint32 = 0x01
	FrameEnd  uint32 = 0x02
)
...
func (w ConnWriter) Write(p []byte) (n int, err error) {
	_, err = FrameWriter(w.conn, []byte(w.key), FrameData)
	if err != nil {
		return 0, err
	}
	return FrameWriter(w.conn, p, FrameData)
}

func (w ConnWriter) Close() (err error) {
	_, err = FrameWriter(w.conn, []byte(w.key), FrameEnd)
	if err != nil {
		return err
	}
	return
...
// 写入数据 每次添加数据类型和数据长度和key
func FrameWriter(conn net.Conn, p []byte, FrameType uint32) (n int, err error) {
	err = binary.Write(conn, binary.BigEndian, FrameType)
	if err != nil {
		return
	}
	length := uint32(len(p))
