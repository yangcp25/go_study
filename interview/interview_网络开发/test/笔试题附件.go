...
// 自定义writer实现 write clone 满足io.WriteCloser接口定义

type WriteCloser struct {
	Conn *Conn
}

func (w *WriteCloser) Write(data []byte) (n int, err error) {
	return writeFrame(w.Conn.Conn, data)
}

func (w *WriteCloser) Close() (err error) {
	_, err = writeFrame(w.Conn.Conn, nil)
	if err != nil {
		return err
	}
	//return w.conn.Close()
...
func (conn *Conn) Send(key string) (writer io.WriteCloser, err error) {
	// 先写入key,在返回这个writer
	_, err = writeFrame(conn.Conn, []byte(key))
	if err != nil {
		return nil, err
	}
	return &WriteCloser{Conn: conn}, nil
}
...
func (conn *Conn) Receive() (key string, reader io.Reader, err error) {
	// 读取key
	keyBuf, err := readFrame(conn.Reader)
	if err != nil {
		return
	}
	return string(keyBuf), &Reader{
		Conn:     conn,
		FrameNum: 0,
	}, nil
}
...
func (r *Reader) Read(p []byte) (n int, err error) {
	...
	if r.FrameNum <= 0 {
		var length uint32
		err = binary.Read(r.Conn.Reader, binary.BigEndian, &length)
		if err != nil {
			return 0, err
		}
		if length == 0 {
	...
	r.FrameNum -= dataL
	return io.ReadFull(r.Conn.Reader, p[:dataL])
}
